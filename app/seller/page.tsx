...
export default function SellerDashboard() {
  ...
  const fetchMenuItems = async () => { try { const r = await fetch('/api/seller/menu', { credentials: 'include' }); const d = await r.json(); if (r.ok) { setMenuItems(d.menuItems || []); setStats(s => ({ ...s, activeMenuItems: d.menuItems?.length || 0 })) } } catch { toast.error('Failed menu') } }
  const fetchStats = async () => { try { const r = await fetch('/api/seller/stats', { credentials: 'include' }); const d = await r.json(); if (r.ok) setStats(d.stats) } catch {} }
  const fetchOrders = async () => { try { const r = await fetch('/api/seller/orders', { credentials: 'include' }); const d = await r.json(); if (r.ok) { setOrders(d.orders || []); calculateStatsFromOrders(d.orders || []) } } catch {} }
